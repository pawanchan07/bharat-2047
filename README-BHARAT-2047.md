...
- a **blockchain voting centre** with real SHA-256 hashing, real proof-of-work
...
**The test a system has to pass** before a chain is worth its cost, all seven and not
...
6. Press **Step inside and bring a problem**. Kamla Devi's pension has stopped.
...
12. Press **Step inside and audit the bank**. Every balance is already sealed; the
...
18. Press **Step inside and verify a degree**, then **Issue her certificate**. Eleven
...
22. **Correct it**: the school got a roll number wrong, or she legally changed her name.
    ...
    it refused. Then watch the challenge window run down, twelve seconds here and weeks in
...
- `src/components/india/panchayat.ts` holds the real engine behind it: training
...
- `src/components/india/bank.ts` holds the real engine behind it: Pedersen commitments
...
- `scripts/scan-typography.mjs`: fails on an em dash, en dash or curly quote, in the
  source and in the docs alike
...
graphics card through [WebLLM](https://github.com/mlc-ai/web-llm), with no key, no account,

...
- "Blockchain" bundles three properties people buy together and almost never all need:
  ...
  decentralised consensus (who may append, among parties who distrust each other, which is expensive
  and really a political question); and trustless value transfer (both of those plus an
  asset, and that is crypto).
...
biometrics, because a fingerprint cannot be revoked.
...
export const FAQ: FaqEntry[] = [
  ...
  },
  {
    ...
    answer:
      'Benford needs the magnitudes of numbers, and a commitment hides magnitude by construction, which is the whole point of it. So the Benford test runs on figures the bank publishes itself, not on the sealed ledger, and the screen says so rather than letting you assume otherwise. It is one of two limits stated openly; the other is that hiding amounts while leaving the transaction graph visible is a real, unsolved tradeoff.',
  },
  {
  ...
  },
  {
    ...
    answer:
      'Nowhere we control, because there is no server here at all. The browser does the recognition. If your device has a language pack installed, the audio is transcribed locally and never leaves the machine, and the badge next to the microphone says so. If it does not, the browser sends the audio to its own speech service to transcribe. That badge tells you which of the two is happening every time.',
  },
  {
